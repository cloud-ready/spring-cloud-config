...
```bash
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app/production.env,ut.env/develop
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app/production.env,ut.env
```

/{application}-{profiles}.json
/{label}/{application}-{profiles}.json
```bash
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app-production.env,ut.env.json
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/develop/demo-app-production.env,ut.env.json
```

/{application}-{profiles}.properties
/{label}/{application}-{profiles}.properties
```bash
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app-production.env,ut.env.properties
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/develop/demo-app-production.env,ut.env.properties
```
...
```bash
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app-production.env,ut.env.yml
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/demo-app-production.env,ut.env.yaml

curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/develop/demo-app-production.env,ut.env.yml
curl -s -X GET -u admin:admin_pass http://standalone.config-server:8888/config/develop/demo-app-production.env,ut.env.yaml
```
...
```bash
curl -s -X POST http://standalone.config-server:8888/config/encrypt -H 'Accept: application/json' -d mysecret
```

4.2. Decrypt
Only admin can access `/decrypt` decrypt
```bash
curl -s -X POST -u admin:admin_pass http://standalone.config-server:8888/config/decrypt -H 'Accept: application/json' -d $(curl -s -X POST http://standalone.config-server:8888/config/encrypt -H 'Accept: application/json' -d mysecret)
```
...
6. Other endpoints
- http://standalone.config-server:8888/

- http://standalone.config-server:8888/config/deployKeyPublic

- http://standalone.config-server:8888/manage/info
- http://standalone.config-server:8888/manage/health
...
> i.e. curl -X GET -u application@staging.env http://standalone.config-server:8888/config/application/staging.env
+ Use `/encryptParentPassword` to generate a encrypted and signed parent password (token), only admin can do this.
```bash
curl -s -X POST http://standalone.config-server:8888/config/encryptParentPassword \
...
```bash
curl -s -X GET -u demo-app:demo-app_pass http://standalone.config-server:8888/config/demo-app/production.env,ut.env/develop
curl -s -X GET -u demo-app:demo-app_pass http://standalone.config-server:8888/config/demo-app/production.env,ut.env/master
```
